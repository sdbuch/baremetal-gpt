...
def _embedding_discrete(config: Config, params: EmbeddingDiscrete, tokens: jax.Array):
  w_cast = params.w.astype(jnp.float32)  # upcast to perform the bwd scatter-add in fp32
  emb = w_cast.at[tokens].get(out_sharding=jax.P(*config.sharding.res_stream))
